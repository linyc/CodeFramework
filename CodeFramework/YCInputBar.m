...
@interface YCInputBar()
    
@property (nonatomic,strong) UIView *viewInputBar;//附加在键盘上方的view
@property (nonatomic,strong) UITextViewPlaceholder *txtInput;//输入框
...
-(YCInputBar*)initBar:(UIView *)mainView sendButtonTitle:(NSString *)title maxTextLength:(NSInteger)length
{
    if (self = [super init]) {
        ...
        //frame隐藏在屏幕下方
        self.viewInputBar = [[UIView alloc] initWithFrame:CGRectMake(0, screenRect.size.height-44, screenRect.size.width, 44)];
        self.viewInputBar.backgroundColor=[UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1];
        
        self.txtInput = [[UITextViewPlaceholder alloc]  initWithFrame:CGRectMake(8, 8, self.viewInputBar.bounds.size.width-70, 28)];
        ...
        self.btnReply.backgroundColor = [UIColor orangeColor];
        [self.btnReply setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [HelperView setViewCorner:self.btnReply corner:4];
...
-(void)removeSelf
{
    [self.viewInputBar removeFromSuperview];
    self.viewInputBar = nil;
}
-(void)sendBtnClick
{
    //主view那边判断完再隐藏键盘
...
//        [self.txtInput resignFirstResponder];
        ...
        self.viewInputBar.frame = CGRectMake(0, screenRect.size.height-44, screenRect.size.width, 44);
    }
}
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    self.txtInput.hidePlaceholderView = newString.length > 0;
    return YES;
}
...
-(void)keyboardHide:(NSNotification*)notification
{
    ...
    CGRect inputRect = self.viewInputBar.frame;
    inputRect.origin.y = CGRectGetMinY(kbRect)-44;
    [self.viewInputBar setFrame:inputRect];
}
-(void)dealloc
{
